    ...
    _createDeployment(region) {
      ...
      return new BbPromise( function( resolve, reject ){
        ...
        let doDeploy = function() {
          ...
          let s_function = _this.project.getFunction(_this.endpoints[0].getFunction().getName());

          let params = {
            restApiId:           _this.restApiData.id, /* required */
            stageName:           _this.evt.options.stage, /* required */
            cacheClusterEnabled: s_function.stage && s_function.stage.cacheClusterEnabled || false,
            cacheClusterSize:    s_function.stage && s_function.stage.cacheClusterSize,
          ...
          _this.provider.request('APIGateway', 'createDeployment', params, _this.evt.options.stage, region)
            .then(function(response) {
              ...
              return _this.provider.request('APIGateway', 'getStage', params, _this.evt.options.stage, region)
              .then(function(response) {
                let patchOperations = [];
                // 継承元設定
                if(s_function.stage && s_function.stage.default){
                  Object.keys(s_function.stage.default).forEach(function (key) {
                    patchOperations.push({
                      op: "replace",
                      path: `/*/*/${key}`,
                      value: `${s_function.stage.default[key]}`
                    });
                  });
                }
                ...
                _this.endpoints.forEach(function(endpoint) {
                  // 存在していたら削除（ステージ設定継承）
                  if(response.methodSettings[`${endpoint.path.replace(/\//g, '~1')}/${endpoint.method}`]){
                    patchOperations.push({
                      op: "remove",
                      path: `/${endpoint.path.replace(/\//g, '~1')}/${endpoint.method}`,
                      value: ""
                    });
                  }
                  Object.keys(endpoint.stage).forEach(function (key) {
                    // 上書き設定
                    patchOperations.push({
                      op: "replace",
                      path: `/${endpoint.path.replace(/\//g, '~1')}/${endpoint.method}/${key}`,
                      value: `${endpoint.stage[key]}`
                    });
                  });
                });
                let params = {
                ...
                return _this.provider.request('APIGateway', 'updateStage', params, _this.evt.options.stage, region)
                .then(function(response) {
