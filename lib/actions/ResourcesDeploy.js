            ...
            return _this._createOrUpdateResourcesStack()
                ...
                .then(cfData => {
                  ...
                  if (typeof cfData === 'string') {
                    ...
                    if (cfData === 'No resource updates are to be performed.') {

                      const project     = S.getProject(),
                          projectName   = project.getName(),
                          stage         = _this.evt.options.stage,
                          aws           = S.getProvider('aws'),
                          stackName     = project.getRegion(stage, _this.evt.options.region).getVariables().resourcesStackName || aws.getResourcesStackName(stage, projectName);
                      return aws.request('CloudFormation', 'describeStacks', {StackName: stackName}, stage, _this.evt.options.region).then(cfStackDatas => {
                          const cfStackData = cfStackDatas.Stacks[0];
                      ...
                      .then(() => {
                        ...
                        region.save();
                        SCli.log(cfData + ":" +  JSON.stringify(region.getVariables()));
                        return;
                      });
                    }
